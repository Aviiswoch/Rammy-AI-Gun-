...
static jlongArray nativeGetIntegrityStats(JNIEnv *env, jclass clazz) {
    uint64_t native_stats[UVC_INTEGRITY_STAT_COUNT] = {0};
    jlong java_stats[UVC_INTEGRITY_STAT_COUNT] = {0};
    int index;
    (void) clazz;

    uvc_get_integrity_stats(native_stats, UVC_INTEGRITY_STAT_COUNT);
    for (index = 0; index < UVC_INTEGRITY_STAT_COUNT; ++index)
        java_stats[index] = (jlong) native_stats[index];

    jlongArray result = env->NewLongArray(UVC_INTEGRITY_STAT_COUNT);
    if (result)
        env->SetLongArrayRegion(result, 0, UVC_INTEGRITY_STAT_COUNT, java_stats);
    return result;
}
