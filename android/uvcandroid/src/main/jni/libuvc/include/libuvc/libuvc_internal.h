...
#if defined(RAMMY_UVC_INTEGRITY_LOGGING) && defined(__ANDROID__)
#include <android/log.h>
#define UVC_INTEGRITY_LOG(format, ...) \
    __android_log_print(ANDROID_LOG_DEBUG, "RammyUvcNative", "[UVC_DROP] " format, ##__VA_ARGS__)
#else
#define UVC_INTEGRITY_LOG(format, ...)
#endif
...
struct uvc_stream_handle {
    ...
    /** True when any packet/transfer belonging to the assembling frame is invalid. */
    uint8_t frame_corrupted;
    /** True only after a valid UVC EOF marker has been observed for this frame. */
    uint8_t frame_saw_eof;
